var CatalogView = {
  ...
  render: function (el, params) {
    ...
    if (isCust) setTimeout(function () { App.showMinOrderMsg(); }, 400);
  },
  ...
  _sectionLabel: function (isCust, isAdmin) {
    ...
    if (CatalogView._section === 'personal') return '<div class="section-label"><span class="material-icons-round">star</span> המוצרים שלך — מחירים אישיים</div>';
  ...
  _getProducts: function () {
    ...
    if (cust && CatalogView._section === 'full') {
      list = list.filter(function (p) { return !App.Pricing.hasPersonal(p, cust); });
    }
    return list;
  },
  ...
  _renderGrid: function () {
    ...
    if (list.length === 0) {
      ...
      grid.innerHTML = list.map(function (p) { return CatalogView._card(p, cust, isAdmin); }).join('');
      if (!App._obs) {
    ...
    if (fullLink && App.Auth.isCustomer() && CatalogView._section === 'personal') {
  ...
  _card: function (product, cust, isAdmin) {
    var inStock = product.stock > 0;
    var showPrice = isAdmin || !!cust;
    var unitPrice = showPrice ? App.Pricing.getUnitPrice(product, isAdmin ? { personalPrices:{}, generalDiscount:0 } : cust) : null;
    if (isAdmin) unitPrice = product.price;
    var discPct = cust ? App.Pricing.getDiscountPct(product, cust) : 0;
    var hasP = !!cust && App.Pricing.hasPersonal(product, cust);

    var priceHTML = showPrice
      ? '<div class="card-price-row">' +
          '<span class="price-symbol">₪</span>' +
          '<span class="price-amount">' + unitPrice + '</span>' +
    ...
    if (cust && inStock) {
      ...
      orderHTML = '<div class="out-of-stock-label"><span class="material-icons-round">remove_circle_outline</span> חסר במלאי</div>';
